...
function validarDatos(data) {
  ...
  if (data.title.length < 20) {
    errores.push('El título debe tener al menos 20 caracteres.');
  }
  ...
  if (data.exercise.length < 20) {
    errores.push('El nombre del ejercicio debe tener al menos 20 caracteres.');
  }

  if (data.comments && data.comments.length > 255) {
    errores.push('Los comentarios deben tener un máximo de 255 caracteres.');
  }
...
export default function TextEditor({ lecture, openModal, emotionalSkill }) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      navigate('/manager/content/');
      openModal('El contenido fue agregado correctamente', 'Éxito');
    } catch (err) {
      openModal(err.message);
    }
  };
